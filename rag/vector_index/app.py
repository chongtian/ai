...
from modules.data_extraction import extract_mock_linkedin_profile
...
def process_profile():
    """Process a LinkedIn profile (from mock data) and generate initial facts.
    ...
    try:
        ...
        profile_data = extract_mock_linkedin_profile()
...
def chat_with_profile(session_id, user_query, chat_history):
    ...
    try:
        ...
        # Update chat history
        return chat_history + [{'role':'user', 'content':user_query.strip()}, {'role':'assistant', 'content':response.response.strip()}] 
