...
class MCPHTTPHostApp(MCPHTTPClient):
    """AI host application that uses Ollama LLM with MCP HTTP server tools."""

    def __init__(self, server_url: str, roots_dir: str, is_debug: bool = False):
        ...
        self.model = "ollama_chat/qwen3.5:9b"  
    ...
    async def get_available_tools(self):
        ...
        for tool in mcp_tools:
            ...
            # Convert MCP input schema to Ollama parameters
    ...
    async def chat(self, user_message: str, history: list):
        ...
        # Call ollama with tools (only pass tools if they exist)
        if tools:
            response = completion(
                model=self.model,
        ...
        if assistant_message.tool_calls:
            ...
            for tool_call in assistant_message.tool_calls:
                ...
                # convert the arguments to dict, as the function does not accept string parameter
    ...
    def create_interface(self):
        ...
        with gr.Blocks(title="MCP HTTP AI Host") as interface:
            gr.Markdown(f"""
            # MCP HTTP AI Host
            Chat with {self.model} using tools from the MCP HTTP server.
